...
public class UtilTimeInterval {
    ...
    public static SimpleDateFormat getSimpleDateFormat() {
        return new SimpleDateFormat(ConstantUtils.Day.FORMAT_DATE_YYMMDDHHMMSS);
    }
    ...
    public static void saveDeepClearShowAdTimes(Context context, int times) {
        SharedPreferences pre = context.getSharedPreferences(ConstantUtils.Day.FORMAT_DATE_YYMMDD, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = pre.edit();
        editor.putInt(ConstantUtils.Day.FORMAT_DATE_YYMMDD, times);
        editor.apply();
    }
    ...
    public static int getDeepClearShowAdTimes(Context context) {
        SharedPreferences pre = context.getSharedPreferences(ConstantUtils.Day.FORMAT_DATE_YYMMDD, Context.MODE_PRIVATE);
        return pre.getInt(ConstantUtils.Day.FORMAT_DATE_YYMMDD, -1);
    }
...
//        timeInterval=1 * 1000;
        ...
        if (!"".equals(autoDate)) {
            try {
                ...
                long nowTime = now.getTime();
                if ((nowTime - dateTime) > (timeInterval)) {
                    String simpleDateFormat = new SimpleDateFormat(ConstantUtils.Day.FORMAT_DATE_YYMMDDHHMMSS).format(new Date());
